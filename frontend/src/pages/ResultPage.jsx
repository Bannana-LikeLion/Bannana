import {
  ...
  function ResultPage() {
    ...
    /*
      roomId / 쿼리파라미터가 바뀌면 바텀시트 상태를 초기값으로 되돌린다.

      react-hooks/set-state-in-effect 규칙에 걸리지만, 여기서는 상태 5개와 ref 1개를 함께
      되돌려야 해서 렌더 중 조정 패턴으로는 옮길 수 없다(ref는 렌더 중에 쓸 수 없음).
      제대로 고치려면 라우트 변경 시 컴포넌트를 key로 리마운트하는 방식이 맞다. 프론트 담당자 확인 필요.
    */
