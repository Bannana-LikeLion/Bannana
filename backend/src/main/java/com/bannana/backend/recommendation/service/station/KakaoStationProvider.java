...
public class KakaoStationProvider implements StationProvider {
    ...
    private final KakaoStationClient kakaoStationClient;
    private final List<Integer> searchRadii;

    public KakaoStationProvider(KakaoStationClient kakaoStationClient, List<Integer> searchRadii) {
        this.kakaoStationClient = kakaoStationClient;
        this.searchRadii = List.copyOf(searchRadii);
    }

    public boolean isConfigured() {
        return kakaoStationClient.isConfigured();
    }
    ...
    public List<Station> findCandidates(GeoPoint center, int minCount, int maxCount) {
        ...
        for (int radius : searchRadii) {
            List<Station> found = kakaoStationClient.searchSubwayStations(center, radius);
